...
bool permitted_name(const std::wstring& name, const std::vector<boost::wregex>& include_patterns, const std::vector<boost::wregex>& exclude_patterns) {
	for(std::vector<boost::wregex>::const_iterator ipit(include_patterns.begin()), ipend(include_patterns.end()); ipit != ipend; ++ipit) {
		if(boost::regex_match(name, *ipit)) {
			for(std::vector<boost::wregex>::const_iterator epit(exclude_patterns.begin()), epend(exclude_patterns.end()); epit != epend; ++epit) {
...
int wmain(int argc, wchar_t* argv[])
{
	try
	{
		...
		for(std::vector<std::wstring>::const_iterator it(inc_patterns.begin()), end(inc_patterns.end()); it != end; ++it) {
		...
		for(std::vector<std::wstring>::const_iterator it(exc_patterns.begin()), end(exc_patterns.end()); it != end; ++it) {
		...
		for(std::vector<std::wstring>::const_iterator it(inc_epatterns.begin()), end(inc_epatterns.end()); it != end; ++it) {
			include_patterns.push_back(boost::wregex(*it, boost::regex::icase));
		}
		for(std::vector<std::wstring>::const_iterator it(exc_epatterns.begin()), end(exc_epatterns.end()); it != end; ++it) {
		...
		for(std::vector<std::wstring>::const_iterator it(directories.begin()), end(directories.end()); it != end; ++it) {
		...
		for(size_map_type::iterator it(files.begin()), end(files.end()); it != end;) {
		...
		for(size_map_type::iterator it(files.begin()), end(files.end()); it != end; ++it) {
			...
			size_t count(0);
			for(std::vector<std::vector<std::wstring> >::const_iterator it(duplicates.begin()), end(duplicates.end()); it != end; ++it) {
